...
def cubes_to_verts(num_verts, cubes, value, reduce='mean'):
    ...
    # 🛡️ THE NUCLEAR FIX: Force target precision to match the model weights (float16)
    # This prevents any upstream "float32 promotion" from causing a crash.
    target_dtype = torch.float16 
    
    reduced = torch.zeros((num_verts, M), device=value.device, dtype=target_dtype)
    src_value = value.to(dtype=target_dtype).reshape(-1, M)
    
    return torch.scatter_reduce(
        reduced, 0, 
        cubes.unsqueeze(-1).expand(-1, -1, M).reshape(-1, M), 
        src_value, 
        reduce=reduce, 
        include_self=False
    )
...
def sparse_cube2verts(coords, feats, training=True):
    ...
    if training:
        con_loss = torch.mean((feats - new_feats[cubes].to(feats.dtype)) ** 2)
    else:
        con_loss = 0.0
